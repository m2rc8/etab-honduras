...
use MINSAL\IndicadoresBundle\Entity\fichaDetalleseg;
...
public function matriztresAction() {
        $array = $this->ParametrosAction();
        return $this->render('IndicadoresBundle:FichaTecnicaAdmin:matriztres.html.twig', $array);
    }
...
//buscar real
                ...
                $vars2['ficha_id']= 'null';
                  ...
                  where fd.tipo= 1 and fd.id_clas= " .  $vars2['id'].
                            " and fd.id_ficha= " . $vars[0]['id']  .
...
$stmt = $em->getConnection()->prepare($sql);
 ...
 public function mesesPeriodo($periodo, $titulo) {
        $conta = 0;
        $meses = array();

        foreach ($titulo as $mesPeriodo) {
            //REPLACE
            if ($conta > 2) {
                $obj = array();
                $parametros = explode("-", $mesPeriodo);
                if ($periodo == $parametros[1]) {
                    array_push($obj, $parametros[0]);
                    array_push($obj, $conta);
...
public function fornpfichastresAction() {
        $request = $this->getRequest();
        $fechaP = $request->request->get('fecha');
        $titulo = $request->request->get('titulo');
        $titulo = explode(",", $titulo);
        $periodos = explode(",", $request->request->get('periodos'));
        ...
        foreach ($periodos as $per) {
            if (empty($per)) {
                ...
                $meses = $this->mesesPeriodo($per, $titulo);
                $idFichaTecnica;
                $idClas;
                $res = $this->removeMatrizsegGestor($periodo, $em);
...
public function removeMatrizsegGestor($periodo, $em) {
        $sql = 'delete from IndicadoresBundle:fichaDetalleseg id where id.periodo =:periodo';
        $query = $em->createQuery($sql);
        $query->setParameter('periodo', $periodo);
        $numDel = $query->execute();
        return $numDel;
...
public function fornpfichaTres($param, $pperiodo) {
        $parametros = explode("&&", $param);
        $fecha = $parametros[0];
        //parametros de rango de fecha 3/4
        ...
        $periodos = array();
        $periodos = $this->periodos($anio1, $interval->format("%y"));
        $intervalAnos = $interval->format("%y") * 12;
        $totalMeses = $intervalAnos + $intervalMeses;
        //echo $interval->format('%R%a días');
        $per = explode("-", $fecha);
        $periodo = $pperiodo; //$per[0];
        $est = $parametros[1];
        $gestor = $est;
        $em = $this->getDoctrine()->getManager(); //
...
// QUERY PARA OBTENER EL NIVEL 1
        ...
        foreach ($indicadores as $vars) {
            ...
            $sql = " select fc.id, fc.descripcion as clasificacion,
                  ft.descripcionm  as indicador, ft.meta, ft.id as idft, ft.tipo, ft.ficha_id,
                  CASE WHEN  (fd.m1) IS NULL THEN  ft.meta  else fd.m1 end as m1,
                  CASE WHEN  (fd.m2) IS NULL THEN  ft.meta  else fd.m2 end as m2,
                  CASE WHEN  (fd.m3) IS NULL THEN  ft.meta  else fd.m3 end as m3,
                  CASE WHEN  (fd.m4) IS NULL THEN  ft.meta  else fd.m4 end as m4,
                ...
                from  clasificacion_tecnicam fc
                left join clasificacion_tecnicai ft on (fc.id = ft.clasificaciontecnicam_id)
                left join ficha_detalleseg fd on
                (fd.id_ficha = ft.id
                and fd.id_clas = fc.id
                and fd.tipo=0
                and fd.periodo='" . $periodo . "')  
                where fc.id= " . $vars['id'] . ""; //"' and fd.gestor= " . $gestor . ")
            $stmt2 = $em->getConnection()->prepare($sql);
            $stmt2->execute();
            $resultadoDos = $stmt2->fetchAll();
            $var2 = array('id' => $vars['id'],
                'descripcion' => substr($vars['descripcion'], 0, 350), 'tipo' => '',
                'ene' => null, 'feb' => null, 'mar' => null, 'abr' => null,
                'may' => null, 'jun' => null, 'jul' => null, 'ago' => null,
                'sep' => null, 'oct' => null, 'nov' => null, 'dic' => null);
            //1
...
//buscar real
                  ...
                  fd.m11 as m11,
                  fd.m12 as m12, observacion  
                  FROM ficha_detalleseg fd 
                  where fd.tipo= 1 and fd.id_clas= " . $vars[0]['id'] .
                            " and fd.id_ficha= " . $vars2['idft'] .
                            " and fd.periodo='" . $periodo . "'"; // . "' and fd.gestor =" . $gestor;
                    ...
                    array_push($indicadorFinal, $v3);
                    //buscar Planificado
                    $var2 = array('id' => $vars2['idft'],
                        'descripcion' => substr($vars2['indicador'], 0, 150), 'tipo' => 'Planificado',
                        'ene' => $vars2['m1'], 'feb' => $vars2['m2'], 'mar' => $vars2['m3'], 'abr' => $vars2['m4'],
                        'may' => $vars2['m5'], 'jun' => $vars2['m6'], 'jul' => $vars2['m7'], 'ago' => $vars2['m8'],
                        'sep' => $vars2['m9'], 'oct' => $vars2['m10'], 'nov' => $vars2['m11'], 'dic' => $vars2['m12'],
                        'observacion' => $vars2['observacion'],
                  ...
                  fd.m12 as m12, observacion  
                  FROM ficha_detalleseg fd 
                  where fd.tipo= 2 and fd.id_clas= " . $vars[0]['id'] .
                            " and fd.id_ficha= " . $vars2['idft'] .
                            " and fd.periodo='" . $periodo . "'"; //. " and fd.gestor =" . $gestor;
                    ...
                    //CASO QUE REAL SEA NULL O CERO
                    //CASO QUE NO EXISTA VALOR PLANIFICADO 
                    $va3 = array('id' => $vars2['idft'],
                        'descripcion' => substr($vars2['indicador'], 0, 150), 'tipo' => 'Status %',
                        'ene' => $vars4[0]['m1'], 'feb' => $vars4[0]['m2'], 'mar' => $vars4[0]['m3'], 'abr' => $vars4[0]['m4'],
                        'may' => $vars4[0]['m5'], 'jun' => $vars4[0]['m6'], 'jul' => $vars4[0]['m7'], 'ago' => $vars4[0]['m8'],
                        'sep' => $vars4[0]['m9'], 'oct' => $vars4[0]['m10'], 'nov' => $vars4[0]['m11'], 'dic' => $vars4[0]['m12'],
                        'observacion' => $vars4[0]['observacion'],
                    ...
                    // que se buscaran en las temporales, para extraer la info real
                    //obtener todos los indicador
                    // ESTATUS ES LO REAL/ PLANIFICADO
                    $sql = "SELECT id, nombre, unidad_medida, meta,formula, campos_indicador "
                            . "FROM ficha_tecnica where  id = " . $vars2['ficha_id'];
                    ...
                    //FORMULA
                    //NUMERO O PORCENTAJE
                    if (strtolower($un) == 'numero' OR strtolower($un) == '#') {
                        $formula = str_replace(' ', '', $vars[0]['formula']);
                        preg_match_all('/\{([\w]+)\}/', $formula, $vars[0]['formula']);
                        $formula = strtolower($formula);
                        $formula = str_replace('{', '(', $formula);
                        ...
                        $formula = str_replace('(', 'sum(', $formula);
                        //$formula = 'sum' . $formula;
                    } else if (strtolower($un) == 'porcentaje' OR strtolower($un) == '%') {
                        $formula = str_replace(' ', '', $vars[0]['formula']);
                        preg_match_all('/\{([\w]+)\}/', $formula, $vars[0]['formula']);
                        $formula = strtolower($formula);
                        $formula = str_replace('{', '(', $formula);
                        $formula = str_replace('}', ')', $formula);
                        $formula = str_replace('(', 'sum(', $formula); //$formula = str_replace('(', 'sum(', $formula);
                        $formula = str_replace('((', '(sum(', $formula);
                        $formula = substr("$formula", 3, strlen($formula)); // devuelve "d"
                        $formula = 'round(' . $formula . '::numeric,2)';
                        //$formula = 'sum' . $formula;
                        //$formula = str_replace('(', 'sum(', $formula); //$formula = str_replace('(', 'sum(', $formula);
                    }
                    //FIN
...
//PARAMETROS: AÑO
                    ...
                    if (strpos($ci, 'gestor') !== false AND $gest != '-- TODOS LOS GESTORES--') {
                        ...
                        $sql = " select " . $varMes . " as mes, " . $formula .
                                " as formula from tmp_ind_" . $id .
                                " WHERE anio = " . $periodo .
                                " group by " . $varMes;            //. " AND gestor ='" . $gest . "'"
                    } else {
                        $sql = " select " . $varMes . " as mes, " . $formula .
                                " as formula from tmp_ind_" . $id .
                                " WHERE anio = " . $periodo .
                    ...
                    foreach ($temp as $temp_) {
                        ...
                        $sql2 = " Select " .
                                ...
                                "WHEN id_color_alerta = 3 then 'rojo' " .
                                "END as color " .
                                "FROM indicador_alertasseg where limite_inferior <= " . $valFormula . " and limite_superior >= " . $valFormula . " and id_indicador =" . $idft;
                        $stmt2 = $em->getConnection()->prepare($sql2);
                        try {
                            $stmt2->execute();
                        } catch (\Exception $e) {
                    ...
                    if ($bandera == 0) {
                        $var2['tipo'] = 'Real';
                        $var2['id'] = $vars2['idft']; //$vars3[0]['id'];
                        //$var2['id'] = 'XX'; //$vars3[0]['id'];
                        $var2['descripcion'] = substr($vars2['indicador'], 0, 150);
                        $var2['observacion'] = '--PRODUCCION--';
                        //4 B
                        array_push($indicadorFinal, $var2);
                        //$formula = $vars['formula'];
                    ...
                    $v3 = array('id' => $vars2['idft'],
                        'descripcion' => substr($vars2['descripcion'], 0, 150), 'tipo' => 'Planificado',
                        'ene' => $vars3[0]['meta'], 'feb' => $vars3[0]['meta'], 'mar' => $vars3[0]['meta'], 'abr' => $vars3[0]['meta'],
                        'may' => $vars3[0]['meta'], 'jun' => $vars3[0]['meta'], 'jul' => $vars3[0]['meta'], 'ago' => $vars3[0]['meta'],
                        'sep' => $vars3[0]['meta'], 'oct' => $vars3[0]['meta'], 'nov' => $vars3[0]['meta'], 'dic' => $vars3[0]['meta'],
                        'observacion' => '--PRODUCCION--',
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $ene . " and limite_superior >= " . $ene . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $feb . " and limite_superior >= " . $feb . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $mar . " and limite_superior >= " . $mar . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $abr . " and limite_superior >= " . $abr . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $may . " and limite_superior >= " . $may . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $jun . " and limite_superior >= " . $jun . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $jul . " and limite_superior >= " . $jul . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $ago . " and limite_superior >= " . $ago . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $sep . " and limite_superior >= " . $sep . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $oct . " and limite_superior >= " . $oct . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $nov . " and limite_superior >= " . $nov . " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $sql2 = " Select " .
                            ...
                            "WHEN id_color_alerta = 3 then 'rojo' " .
                            "END as color " .
                            "FROM indicador_alertasseg where limite_inferior <= " . $dic .
                            " and limite_superior >= " . $dic .
                            " and id_indicador =" . $vars2['idft'];
                    $stmt2 = $em->getConnection()->prepare($sql2);
                    try {
                        $stmt2->execute();
                    } catch (\Exception $e) {
                    ...
                    $temp_2 = $stmt2->fetchAll();
                    $diccolor = $temp_2[0]['color'];



                    $v4 = array('id' => $vars2['idft'],
                        ...
                        'sep' => $sep, 'oct' => $oct,
                        'nov' => $nov, 'dic' => $dic,
                        'epintar' => $enecolor, 'fpintar' => $febcolor,
                        'mpintar' => $marcolor, 'apintar' => $abrcolor,
                        'mapintar' => $maycolor, 'jpintar' => $juncolor,
                        'jupintar' => $julcolor, 'agpintar' => $agocolor,
                        'spintar' => $sepcolor, 'opintar' => $octcolor,
                        'npintar' => $novcolor, 'dpintar' => $diccolor,
                        'observacion' => '--PRODUCCION--',
                    );
                    array_push($indicadorFinal, $v4);
                }
                $cont++;
                //$id = $vars['id'];
                //$un = $vars['unidad_medida'];
        ...
        if (!$indicadorFinal) {
            ...
            $indicadorFinal2 = array();
            foreach ($indicadorFinal as $tag) {
                $tags = array();
                $tags['id'] = $tag['id'];
                $tags['descripcion'] = $tag['descripcion'];
                $tags['tipo'] = $tag['tipo'];
                $tags['1-' . strval($pperiodo)] = $tag['ene'];
                $tags['2-' . strval($pperiodo)] = $tag['feb'];
                $tags['3-' . strval($pperiodo)] = $tag['mar'];
                $tags['4-' . strval($pperiodo)] = $tag['abr'];
                $tags['5-' . strval($pperiodo)] = $tag['may'];
                $tags['6-' . strval($pperiodo)] = $tag['jun'];
                $tags['7-' . strval($pperiodo)] = $tag['jul'];
                $tags['8-' . strval($pperiodo)] = $tag['ago'];
                $tags['9-' . strval($pperiodo)] = $tag['sep'];
                $tags['10-' . strval($pperiodo)] = $tag['oct'];
                $tags['11-' . strval($pperiodo)] = $tag['nov'];
                $tags['12-' . strval($pperiodo)] = $tag['dic'];
                $tags['c1-' . strval($pperiodo)] = $tag['epintar'];
                $tags['c2-' . strval($pperiodo)] = $tag['fpintar'];
                $tags['c3-' . strval($pperiodo)] = $tag['mpintar'];
                $tags['c4-' . strval($pperiodo)] = $tag['apintar'];
                $tags['c5-' . strval($pperiodo)] = $tag['mapintar'];
                $tags['c6-' . strval($pperiodo)] = $tag['jpintar'];
                $tags['c7-' . strval($pperiodo)] = $tag['jupintar'];
                $tags['c8-' . strval($pperiodo)] = $tag['agpintar'];
                $tags['c9-' . strval($pperiodo)] = $tag['spintar'];
                $tags['c10-' . strval($pperiodo)] = $tag['opintar'];
                $tags['c11-' . strval($pperiodo)] = $tag['npintar'];
                $tags['c12-' . strval($pperiodo)] = $tag['dpintar'];
                $tags['observacion'] = $tag['observacion'];
                array_push($indicadorFinal2, $tags);
            }
        }
        return $indicadorFinal2;
        //$array = $this->ParametrosAction();
        //return $this->render('IndicadoresBundle:FichaTecnicaAdmin:formN.html.twig', $array);
    }
 ...
 public function periodos($pinicial, $cperidos) {
        $cadena = array();
        $valor = intval($pinicial);
        //$cadena = strval($valor);
        array_push($cadena, $valor);
        for ($i = 0; $i < intval($cperidos); $i++) {
            $valor = $valor + 1;
...
public function fornpfichatresAction($param) {
        $parametros = explode("&&", $param);
        $fecha = $parametros[0];
        //parametros de rango de fecha 3/4
        $fechaDesde = "01-" . $parametros[3];
        ...
        $indicadorFinal1 = array();
        $temp = array();
       
        $indicadorFinal4 = array();
        $valInicial = 0;
       
        foreach ($periodos as $periodo) {
            $strPeriodos = strval($strPeriodos) . strval($periodo) . ',';
            $indicadorFinal3 = array();
             $indicador = array();
                           
            $indicadorFinal1 = $this->fornpfichaTres($param, $periodo);
            //array_push($indicadorFinal2, $indicadorFinal1);
            foreach ($indicadorFinal1 as $indicador) {
                //if($valInicial==0){
                $temp=array();
                $temp['id'] = $indicador['id'];
                $temp['descripcion'] = $indicador['descripcion'];
                $temp['tipo'] = $indicador['tipo'];
                $temp['observacion'] = $indicador['observacion']; 
                $temp['c-1-' . strval($periodo)] =  $indicador['c1-' . strval($periodo)];
                $temp['c-2-' . strval($periodo)] =  $indicador['c2-' . strval($periodo)];
                $temp['c-3-' . strval($periodo)] =  $indicador['c3-' . strval($periodo)];
                $temp['c-4-' . strval($periodo)] =  $indicador['c4-' . strval($periodo)];
                $temp['c-5-' . strval($periodo)] =  $indicador['c5-' . strval($periodo)];
                $temp['c-6-' . strval($periodo)] =  $indicador['c6-' . strval($periodo)];
                $temp['c-7-' . strval($periodo)] =  $indicador['c7-' . strval($periodo)];
                $temp['c-8-' . strval($periodo)] =  $indicador['c8-' . strval($periodo)];
                $temp['c-9-' . strval($periodo)] =   $indicador['c9-' . strval($periodo)];
                $temp['c-10-' . strval($periodo)] =   $indicador['c10-' . strval($periodo)];
                $temp['c-11-' . strval($periodo)] =   $indicador['c11-' . strval($periodo)];
                $temp['c-12-' . strval($periodo)] =   $indicador['c12-' . strval($periodo)];
                $temp['1-' . strval($periodo)] = $indicador['1-' . strval($periodo)];
                $temp['2-' . strval($periodo)] = $indicador['2-' . strval($periodo)];
                $temp['3-' . strval($periodo)] = $indicador['3-' . strval($periodo)];
                $temp['4-' . strval($periodo)] = $indicador['4-' . strval($periodo)];
                $temp['5-' . strval($periodo)] = $indicador['5-' . strval($periodo)];
                $temp['6-' . strval($periodo)] = $indicador['6-' . strval($periodo)];
                $temp['7-' . strval($periodo)] = $indicador['7-' . strval($periodo)];
                $temp['8-' . strval($periodo)] = $indicador['8-' . strval($periodo)];
                $temp['9-' . strval($periodo)] = $indicador['9-' . strval($periodo)];
                $temp['10-' . strval($periodo)] = $indicador['10-' . strval($periodo)];
                $temp['11-' . strval($periodo)] = $indicador['11-' . strval($periodo)];
            ...
            if (!$indicadorFinal4) {
                ...
                $temp2=array();
                foreach ($indicadorFinal3 as $temp2) {
                     $temp3=array();
                     $pos =0;
                     foreach ($indicadorFinal4 as $temp3) {
                          if ($temp2['id'] == $temp3['id'] and
                        $temp2['descripcion'] == $temp3['descripcion'] and
                        $temp2['tipo'] == $temp3['tipo']) {
                        $indicadorFinal4[$pos]['1-' . strval($periodo)] = $temp2['1-' . strval($periodo)];      
                        $indicadorFinal4[$pos]['2-' . strval($periodo)] = $temp2['2-' . strval($periodo)];
                        $indicadorFinal4[$pos]['3-' . strval($periodo)] = $temp2['3-' . strval($periodo)];
                        $indicadorFinal4[$pos]['4-' . strval($periodo)] = $temp2['4-' . strval($periodo)];
                    }
                    $pos =$pos+1;
                     }
                }
        ...
        $response->setData(array('message' => $indicadorFinal4,
            ...
            'titulo' => $this->nombresEncabezados($totalMeses, $anio1))
        );
        return $response;
    }


  public function nombresEncabezados($cant, $per) {
        $titulo = '';
        $k = 0;
        $j = 0;
        for ($i = 0; $i <= $cant; $i++) {
        ...
        $titulo = 'ID,Indicadores, ,' . $titulo . 'observacion';
//'hcol'=> 'ID, Indicadores, , , , , , , , , , , , , ,',
        return $titulo;
    }
...
public function reporterexceltresAction($param) {
         $parametros = explode("&&", $param);
        $fecha = $parametros[0];
        //parametros de rango de fecha 3/4
        $fechaDesde = "01-" . $parametros[3];
       ...
       // $response = new JsonResponse();
        $Establecimiento = $indicadorFinal4;
        $encoders = array(new XmlEncoder(), new JsonEncoder());
        $normalizers = array(new GetSetMethodNormalizer());
        $serializer = new Serializer($normalizers, $encoders);
        $jsonContent = $serializer->serialize($Establecimiento, 'json');
...
$sql='SELECT Distinct(ue.idEstablecimiento) AS IDE, 
	...
	//$sql='SELECT es.idEstablecimiento, es.descripcion
	  ...
	  $sqlxx="SELECT f3.id, f3.username  FROM fos_user_user_group f1
		...
		return array(
                    'hcol'=> 'ID, Indicadores, , , , , , , , , , , , , , observacion',
                    'usuario'=>$usuario,
		    'usuarios'=>$usuarios,
		    'usuariosf'=>$usuariosf,
		    'usuariosh'=>$usuariosh,
